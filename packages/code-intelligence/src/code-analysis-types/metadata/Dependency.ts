...
export class DependencyValidator {
   ...
   */
  static validate(dependency: Partial<IDependency>): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
...
export class DependencyUtils {
   ...
   */
  static findCriticalPath(
    taskIds: string[], 
    dependencies: IDependency[], 
    taskEfforts: Map<string, number>
  ): { path: string[]; totalEffort: number } {
    const graph = new Map<string, string[]>();
    ...
    const calculateLongestPath = (taskId: string, visited: Set<string>): number => {
