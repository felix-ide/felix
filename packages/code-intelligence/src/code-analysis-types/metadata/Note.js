...
export class NoteUtils {
    ...
    static fromDbRow(row) {
        ...
        if (hasLinks) {
            note.links = {};
            if (row.stable_links)
                note.links.stable_links = JSON.parse(row.stable_links);
            if (row.fragile_links)
                note.links.fragile_links = JSON.parse(row.fragile_links);
            if (row.semantic_context)
                note.links.semantic_context = row.semantic_context;
        }
        // Handle tags
        ...
        if (hasTags) {
            note.tags = {};
            if (row.stable_tags)
                note.tags.stable_tags = JSON.parse(row.stable_tags);
            if (row.auto_tags)
                note.tags.auto_tags = JSON.parse(row.auto_tags);
            if (row.contextual_tags)
                note.tags.contextual_tags = JSON.parse(row.contextual_tags);
        }
     ...
     */
    static extractAutoTags(content) {
        const tags = [];
        // Extract #hashtags
     ...
     */
    static calculateDepthLevel(parentId, getNoteFunction) {
        if (!parentId) {
     ...
     */
    static buildHierarchySummary(notes) {
        const noteMap = new Map();
        const rootNotes = [];
        // Create note map
        for (const note of notes) {
    ...
    static mergeNotes(notes) {
        ...
        if (hasAnyTags) {
            merged.tags = {};
            if (allStableTags.length > 0)
                merged.tags.stable_tags = [...new Set(allStableTags)];
            if (allAutoTags.length > 0)
                merged.tags.auto_tags = [...new Set(allAutoTags)];
            if (allContextualTags.length > 0)
                merged.tags.contextual_tags = [...new Set(allContextualTags)];
        }
